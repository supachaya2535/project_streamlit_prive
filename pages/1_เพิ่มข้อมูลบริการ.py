...
with st.container():
    ...
    if st.button('เพิ่มข้อมูล'):
        ...
        product_category_df.to_csv(f"./database/product_category/product_category_{date.today().strftime('%Y-%m')}_data.csv",header = True,index = False, encoding="utf-8-sig")
...
with st.container():
    ...
    if st.button('บันทึกการเปลี่ยนแปลง'):
        st.balloons()
        edited_df.to_csv(f"./database/product_category/product_category_{date.today().strftime('%Y-%m')}_data.csv",header = True,index = False, encoding="utf-8-sig")
        edited_df.to_csv(f"./database/product_category/product_category_data.csv",header = True,index = False,encoding="utf-8-sig")
